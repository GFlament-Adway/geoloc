...
if __name__ == "__main__":
    ...
    map = Basemap(projection='merc', llcrnrlat=np.min(gps_coordinates.T[0]) - 1, urcrnrlat=np.max(gps_coordinates.T[0]) + 1, \
                  llcrnrlon=np.min(gps_coordinates.T[1]) - 1, urcrnrlon=np.max(gps_coordinates.T[1]) + 1, lat_ts=20, resolution='l')
    # map.bluemarble()
    map.drawcoastlines()
    map.drawcountries()
